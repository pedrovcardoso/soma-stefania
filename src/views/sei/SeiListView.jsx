...
  const filterInputClass = "relative w-full h-[42px] border border-border rounded-xl cursor-pointer flex items-center justify-between px-4 py-2 text-sm transition-all duration-200 bg-surface hover:border-text-muted/40 hover:bg-gray-50 shadow-sm text-text outline-none focus:border-accent focus:shadow-[0_0_0_3px_rgba(var(--accent-rgb),0.1)]";

  const isAnyLoading = isFiltersLoading || isLoading;
...
              showSearch={false}
              options={[
                "Qualquer data", "Hoje", "Esta Semana", "Semana Passada", 
                "Este Mês", "Mês Passado", "Período Específico..."
              ]}
              value={[
...
              }}
              isLoading={isAnyLoading} // <-- Aplicado aqui também!
            />
          </div>

          {filters.datePreset === 'specific' && (
            <div className="flex-grow min-w-[180px] flex flex-col gap-1.5">
              <label className="text-[11px] font-bold uppercase tracking-wider text-text-muted/80 ml-1">Datas</label>
              
              {/* Lógica do Skeleton no Datepicker */}
              {isAnyLoading ? (
                <div className={`${filterInputClass} !cursor-default hover:bg-surface hover:border-border`}>
                  <div className="flex items-center w-full gap-2">
                    <div className="h-4 w-32 bg-slate-200 animate-pulse rounded" />
                  </div>
                  <MdExpandMore className="text-text-muted/50 shrink-0 ml-2" size={22} />
                </div>
              ) : (
...
                        className={`
                          ${filterInputClass} 
                          ${open ? '!border-accent !shadow-[0_0_0_3px_rgba(var(--accent-rgb),0.1)] !bg-surface' : ''}
                        `}
                      >
                        <div className='flex items-center gap-2 truncate w-full overflow-hidden'>
                          <MdCalendarToday className="text-text-muted/70 shrink-0" size={18} />
                          {filters.dateRange.from ? (
                            <span className="text-text truncate leading-none">
                              {format(filters.dateRange.from, 'dd/MM/yyyy')} - {filters.dateRange.to ? format(filters.dateRange.to, 'dd/MM/yyyy') : ''}
                            </span>
                          ) : (
                            <span className="text-text-muted/70 truncate">Selecione...</span>
                          )}
                        </div>
                        <MdExpandMore 
                          className={`text-text-muted/50 transition-transform duration-300 shrink-0 ml-2 ${open ? 'rotate-180 text-accent' : ''}`} 
                          size={22} 
                        />
                      </Popover.Button>
...
                      >
                        <Popover.Panel className="absolute z-20 mt-[6px] bg-surface border border-border rounded-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                          <DayPicker 
                            mode="range" 
...
              showSelectAll={true}
              showClear={true}
              isLoading={isAnyLoading} // <-- Aplicado aqui
            />
          </div>
